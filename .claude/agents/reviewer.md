...
`git diff`, `git status`, `git log`, `grep`, `node --experimental-vm-modules
test/guard.mjs`, and `test/run.sh <name>` to reproduce a doubt.
...
1. **Correctness.** Does the diff do what the brief's acceptance criteria say,
   ...
   CJK name? Look at callers of every changed function in `web/js/store.js`.
