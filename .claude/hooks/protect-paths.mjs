...
//   web/js/                  build output; the source is src/
...
const rules = [
  ...
  // web/js/ is not in git, so an edit here is lost by the next `npm run build`
  // and never reaches production at all — the deploy builds from src/. Worse,
  // it looks like it worked: the served file really does change until the next
  // build wipes it.
  [/^web\/js\//, 'web/js/ is generated by build.mjs, edit src/ instead. It is not in git, so a change here is silently discarded by the next `npm run build` and never deploys.'],
